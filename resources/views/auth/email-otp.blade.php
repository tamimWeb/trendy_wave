                    ...
                    <div class="axil-signin-form-wrap">
                        <div class="axil-signin-form">
                            <div class="mb-5 alert alert-success">
                                OTP has been sent to your email address({{Session::get('email')}})
                            </div>
                            <p class="b2 mb--55">Enter OTP From Email:</p>
                            <form class="singin-form" method="POST" action="{{ route('email-otp.verify') }}">
                                @csrf
                                <div class="form-group">
                                    <label>OTP</label>
                                    <input type="text" class="form-control" name="email_otp" placeholder="Enter OTP"
                                        required>
                                </div>
                                <div class="form-group">
                                    <button type="submit" class="axil-btn btn-bg-primary submit-btn">Verify
                                        OTP</button>
                                </div>
                            </form>
                        </div>
