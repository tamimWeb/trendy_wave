                ...
                <div class="axil-signin-form-wrap">
                    <div class="axil-signin-form">
                        <div class="mb-5 alert alert-success">
                            OTP has been sent to your Phone({{ Session::get('phone') }})
                        </div>
                        <p class="b2 mb--55">Enter OTP From Phone:</p>
                        <form class="singin-form" method="POST" action="{{ route('phone-otp.verify') }}">
                            @csrf
                            <div class="form-group">
                                <label>OTP</label>
                                <input type="text" class="form-control" name="phone_otp" placeholder="Enter OTP"
                                    required>
                            </div>
                            <div class="form-group">
                                <button type="submit" class="axil-btn btn-bg-primary submit-btn">Verify OTP</button>
                            </div>
                        </form>
                    </div>
                </div>
